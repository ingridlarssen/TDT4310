...
def Task4b():																																									#
    ...
    tagwords, tags = probDist(brown)
    probability = tagwords["START"].prob("START")* \
                  tags["START"].prob("PP") * \
                  tagwords["PP"].prob("I")* \
                  tags["PP"].prob("VB")* \
                  tagwords["VB"].prob("conduct")* \
                  tags["VB"].prob("PP")* \
                  tagwords["PP"].prob("my")* \
                  tags["PP"].prob("NN") * \
                  tagwords["NN"].prob("conduct")* \
                  tagwords["END"].prob("END")
    print("hello",probability)



# P(PP|START) * P(I | PP) * P(VB | PP) * P(like | VB) *	P(PP | VB)  * P(my | PP) * P(NN | PP) * P(house | NN) * P(END | NN)
